...
public class GDGurobi extends Algorithm<GDSolution, GDInstance> {

    private static final Logger log = LoggerFactory.getLogger(GDGurobi.class);
    ...
    protected GRBEnv initializeEnvironment() throws GRBException {
        ...
        env.set(GRB.DoubleParam.TimeLimit, GDGurobi.DEFAULT_TIME_LIMIT);
    ...
    protected DataGurobi initializeModel(GRBModel model, GDInstance instance) throws GRBException {
        ...
        // RESTRICCIÓN 3S: SI I ESTÁ DELANTE DE J, J ESTÁ DELANTE DE K, ENTONCES K NO PUEDE ESTÁR DELANTE DE I
        for (int h = 0; h < numLayers; h++) {
            for (int i = 0; i < numVerticesInLayer; i++) {
                for (int j = i + 1; j < numVerticesInLayer; j++) {
                    for (int k = j + 1; k < numVerticesInLayer; k++) {
                        GRBLinExpr r3s = new GRBLinExpr();
                        System.out.print((dictionaryHP_V.get(h).get(i)+1)+" ");
                        System.out.print((dictionaryHP_V.get(h).get(j)+1)+" ");
                        System.out.print((dictionaryHP_V.get(h).get(k)+1)+"\n");
                        r3s.addTerm(1.0, x[h][i][j]);
                        r3s.addTerm(1.0, x[h][j][k]);
                        r3s.addTerm(1.0, x[h][k][i]);
                        model.addConstr(r3s, GRB.GREATER_EQUAL, 1.0, "r3s_" + h + "_" + i + "_" + j + "_" + k);
                    }
                }
            }
        }
        ...
        for (GDInstance.Edge e : instance.getListOfLongEdges()) {
            ...
            for (GDInstance.Edge edge : listOfEdges) {
                var v = edge.target();
