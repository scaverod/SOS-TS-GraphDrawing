...
public class GDGurobiRafaMultipleLongEdgesRetricted extends Algorithm<GDSolution, GDInstance> {

    private static final Logger log = LoggerFactory.getLogger(GDGurobiRafaMultipleLongEdgesRetricted.class);
    ...
    public GDGurobiRafaMultipleLongEdgesRetricted() {
        try {
    ...
    protected GRBEnv initializeEnvironment() throws GRBException {
        ...
        env.set(GRB.DoubleParam.TimeLimit, GDGurobiRafaMultipleLongEdgesRetricted.DEFAULT_TIME_LIMIT);
    ...
    protected DataGurobi initializeModel(GRBModel model, GDInstance instance) throws GRBException {
        ...
        for (GDInstance.Edge e : instance.getListOfLongEdges()) {
            var source = e.source();
            var adjacentTargetDummies = instance.getAdjacentDummyVerticesOfLongEdgeOfVertex(source, GDInstance.TYPE_OF_ADJACENT_VERTEX.TARGET);
            for (Integer dummy : adjacentTargetDummies) {
                for (Integer otherDummy : adjacentTargetDummies) {
                    if (dummy > otherDummy) {
                        var hp_d = dictionaryV_HP.get(dummy);
                        var hp_od = dictionaryV_HP.get(otherDummy);
                        GRBLinExpr dummyR = new GRBLinExpr();
                        for (int i = 0; i < instance.NumVerticesInLayer(); i++) {
                            if (i != hp_d.getValue1()) {
                                dummyR.addTerm(1.0, x[hp_d.getValue0()][i][hp_d.getValue1()]);
                            }
                            if (i != hp_od.getValue1()) {
                                dummyR.addTerm(-1.0, x[hp_od.getValue0()][i][hp_od.getValue1()]);
                            }
                        }
                        model.addConstr(dummyR, GRB.GREATER_EQUAL, -1 * adjacentTargetDummies.size()+1, "dm_" + dummy + "_" +otherDummy);
                        model.addConstr(dummyR, GRB.LESS_EQUAL, adjacentTargetDummies.size()-1, "dM_" + dummy + "_" + otherDummy);
                    }
                }
            }
            var target = e.target();
            var adjacentSourceDummies = instance.getAdjacentDummyVerticesOfLongEdgeOfVertex(target, GDInstance.TYPE_OF_ADJACENT_VERTEX.SOURCE);
            for (Integer dummy : adjacentSourceDummies) {
                for (Integer otherDummy : adjacentSourceDummies) {
                    if (dummy > otherDummy) {
                        var hp_d = dictionaryV_HP.get(dummy);
                        var hp_od = dictionaryV_HP.get(otherDummy);
                        GRBLinExpr dummyR = new GRBLinExpr();
                        ...
                        model.addConstr(dummyR, GRB.GREATER_EQUAL, -1 * adjacentSourceDummies.size()+1, "dm_" + dummy + "_" +otherDummy);
                        model.addConstr(dummyR, GRB.LESS_EQUAL, adjacentSourceDummies.size()-1, "dM_" + dummy + "_" + otherDummy);
                    }
                }
            }
        }
